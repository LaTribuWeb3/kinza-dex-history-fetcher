...
async function FetchPriceHistory(fetchConfig, currentBlock, web3Provider) {
    ...
    const lastFetchFileName = path.join(DATA_DIR, 'precomputed', 'price', 'wombat', `${fetchConfig.poolName}-lastfetch.json`);
    ...
    if (fs.existsSync(lastFetchFileName)) {
        ...
        for(const pair of fetchConfig.pairs) {
            fs.rmSync(path.join(DATA_DIR, 'precomputed', 'price', 'wombat', `${pair.token0}-${pair.token1}-unified-data.csv`), {force: true});
            fs.rmSync(path.join(DATA_DIR, 'precomputed', 'price', 'wombat', `${pair.token1}-${pair.token0}-unified-data.csv`), {force: true});
        }
    }
