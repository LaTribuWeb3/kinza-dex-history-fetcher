      ...
      if (lastValue && blockNum != startBlock) {
        ...
        for (let i = 0; i < dataContents.poolTokens.length - 1; i++) {
          dataContents.reserveValues[lastValueBlock].priceScale.push(
            beforeValueSplitted[i + 5 + dataContents.poolTokens.length]
          );
        }
...
// this is the old function: avg slippage
function computeLiquidityForSlippageCurvePoolCryptoV2(
  baseQty,
  targetPrice,
  reserves,
  i,
 ...
 * @param {string[]} reserves
 */
function computePriceAndSlippageMapForReserveValueCryptoV2(
  fromSymbol,
  toSymbol,
  poolTokens,
  ampFactor,
...
async function test_new_dy() {
  ...
  const precisions = [1000000000000n, 10000000000n, 1n];
  ...
  const slippageMap = computePriceAndSlippageMapForReserveValueCryptoV2(
    'WETH',
    'USDT',
    ['USDT', 'WBTC', 'WETH'],
    A.toString(),
